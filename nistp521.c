...
#include "ecp_nistp521_core.c"
#include "ecp_nistp521_util.c"

#include "nistp521_macros.h"
                  ...
                  mpz_t scalar)
{
  ...
  vec_jmul_nistp521_inner(rx, ry, rz,
                          curve,
                          x, y, z,
                          scalar);
                    ...
                    mpz_t scalar)
{
  ...
  vec_jmulsw_nistp521_inner(rx, ry, rz,
                            curve,
                            x, y, z,
                            scalar);
  felem_to_mpz_t(RX, rx);
...
void
vec_jsmul_nistp521(mpz_t RX, mpz_t RY, mpz_t RZ,
                   vec_curve *curve,
                   mpz_t *X, mpz_t *Y, mpz_t *Z,
                   mpz_t *scalars,
                   size_t len)
{
  ...
  vec_jsmul_nistp521_inner(rx, ry, rz,
                           curve,
                           x, y, z,
                           scalars,
                           len);
                           ...
                           size_t len)
{
  ...
  ptr.nistp521 =
    (vec_jfmul_tab_nistp521_inner*)
    malloc(sizeof(vec_jfmul_tab_nistp521_inner));

  mpz_t_to_felem(x, X);
  mpz_t_to_felem(y, Y);
  mpz_t_to_felem(z, Z);

  vec_jfmul_init_nistp521_inner(ptr.nistp521, curve, len);

  vec_jfmul_prcmp_nistp521_inner(curve, ptr.nistp521, x, y, z);
                   ...
                   mpz_t scalar)
{
  ...
  vec_jfmul_cmp_nistp521_inner(rx, ry, rz,
                               curve, ptr.nistp521,
                               scalar);
...
{
  vec_jfmul_clear_free_nistp521_inner(ptr.nistp521);
}
